...
// Configure multer for file uploads (memory storage for imgBB upload)
...
async function uploadToImgBB(fileBuffer, mimeType) {
    try {
        const IMGBB_API_KEY = process.env.IMGBB_API_KEY;
        
        if (!IMGBB_API_KEY) {
            throw new Error('IMGBB_API_KEY environment variable is not set. Please set it in Render dashboard or .env file.');
        }

        // Convert buffer to base64 string
        const base64Image = fileBuffer.toString('base64');
        
        // imgBB API accepts base64 as a string parameter
        // Use URLSearchParams for form-urlencoded format
        const params = new URLSearchParams();
        params.append('key', IMGBB_API_KEY);
        params.append('image', base64Image);

        // Upload to imgBB using form-urlencoded format
        ...
        if (!response.ok) {
            ...
            console.error('ImgBB API Error - Status:', response.status);
        ...
        if (result.success === true && result.data && result.data.url) {
            console.log('✅ Image successfully uploaded to imgBB:', result.data.url);
            return result.data.url;
        } else {
            // Handle API errors
            const errorMsg = result.error?.message || result.error?.code || JSON.stringify(result);
            console.error('❌ ImgBB upload failed:', errorMsg);
            console.error('Full response:', JSON.stringify(result, null, 2));
            throw new Error(`Failed to upload image to imgBB: ${errorMsg}`);
        }
    } catch (error) {
        console.error('❌ ImgBB upload error:', error.message);
        if (error.message.includes('fetch')) {
            throw new Error('Network error: Could not connect to imgBB API. Please check your internet connection.');
        }
        throw error;
    }
}
...
app.post('/api/students/add', upload.single('photo'), async (req, res) => {
    try {
        ...
        // Upload photo to imgBB if file exists
        let uploadedPhotoUrl = null;
        if (req.file && req.file.buffer) {
            try {
                uploadedPhotoUrl = await uploadToImgBB(req.file.buffer, req.file.mimetype);
            } catch (imgBBErr) {
                console.error('Failed to upload photo to imgBB:', imgBBErr);
                return res.status(500).json({ error: 'Failed to upload photo: ' + imgBBErr.message });
            }
        }
            ...
            db.get("SELECT id_number FROM students WHERE exam_number = ? AND id_number != ? LIMIT 1", [finalExamNumber, id_number], (checkErr1, checkRow1) => {
                ...
                db.get("SELECT id, id_number, exam_type FROM students WHERE exam_number = ? AND id_number = ? AND exam_type = ?", [finalExamNumber, id_number, exam_type], (checkErr2, checkRow2) => {
                    ...
                    db.run(
                        ...
                        [student_name, level || null, exam_link || null, JSON.stringify(finalSubjectsToStore), totalMarks, maxMarks, computedGrade, examDate, uploadedPhotoUrl || null, existingId],
                            ...
                            if (duplicateError) {
                                ...
                                db.all("SELECT id, id_number, exam_type FROM students WHERE exam_number = ?", [finalExamNumber], (checkErr3, checkRows3) => {
                                    ...
                                    if (sameExamType) {
                                        ...
                                        db.run(
                                            `UPDATE students SET student_name = ?, level = ?, exam_link = ?, subjects_json = ?, total_marks = ?, max_marks = ?, grade = ?, exam_date = ?, photo_url = COALESCE(?, photo_url) WHERE id = ?`,
                                            [student_name, level || null, exam_link || null, JSON.stringify(finalSubjectsToStore), totalMarks, maxMarksForUpdate, computedGradeForUpdate, examDate, uploadedPhotoUrl || null, sameExamType.id],
                                            async function(updateErr) {
                                                if (updateErr) {
...
// Test imgBB connection endpoint
    ...
    try {
        ...
        if (!IMGBB_API_KEY) {
            return res.status(400).json({
                ...
                message: 'Please set IMGBB_API_KEY in Render dashboard → Environment section'
            });
        }
        ...
        if (!response.ok) {
            ...
            return res.status(400).json({
                success: false,
                error: `imgBB API error (${response.status})`,
                message: errorText,
                apiKeySet: true,
                apiKeyLength: IMGBB_API_KEY.length
            });
        }
        ...
        if (result.success && result.data && result.data.url) {
            return res.json({
                success: true,
                message: 'imgBB connection successful!',
            ...
            return res.status(400).json({
                ...
                message: result.error?.message || result.error?.code || 'Unknown error',
                response: result,
                apiKeySet: true,
                apiKeyLength: IMGBB_API_KEY.length
            });
        ...
        return res.status(500).json({
            success: false,
            error: error.message,
            message: 'Failed to connect to imgBB API'
        });
    }
});
